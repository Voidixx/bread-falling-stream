class LiveStreamInteraction {
    ...
    setupElements() {
        ...
        // Bread crumbs background effect removed to reduce lag
        
        // Start fallback timer for bread video when no subscribers
    ...
    async getChannelStats() {
        ...
        try {
            ...
            if (data.error) {
                ...
                if (data.error.reason === 'channelNotFound') {
                    console.warn('Channel not found, checking if channel ID is correct...');
                    this.showError('Channel not found. Please check your channel ID in config.json');
                }
            ...
            // Don't throw error immediately, try to continue with cached data
            return null;
        }
    }
    ...
    startMonitoring() {
        ...
        // Check for changes every 45 seconds to reduce API load and lag
    ...
    async checkForChanges() {
        try {
            // Check subscriber count every cycle
            const stats = await this.getChannelStats();
            ...
            if (stats && stats.subscriberCount) {
                ...
                console.warn('No channel stats received, retrying next cycle...');
            }
    ...
    handleNewSubscriber(count = 1) {
        ...
        // Reset fallback timer since we have subscriber activity
        this.lastSubscriberActivity = now;
    ...
    startFallbackTimer() {
        this.lastSubscriberActivity = Date.now();
        this.fallbackTimer = setInterval(() => {
            const now = Date.now();
            const timeSinceLastSubscriber = now - this.lastSubscriberActivity;
            
            // If no subscriber activity for 1 minute (60000ms), play bread video
            if (timeSinceLastSubscriber >= 60000) {
                console.log('No subscriber activity for 1 minute, playing fallback bread video');
                this.handleNewSubscriber(0); // Play with count 0 to indicate fallback
                this.lastSubscriberActivity = now; // Reset timer
            }
